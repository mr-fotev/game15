    ...
    (xyz-panel :items
               (conj
                (vec (for [i (range s), j (range s), :let [d (+ (* s i) j)], :when (not (= d (:pos @x)))]
                        (button :text (str (brd d)) :foreground :white :font "ARIAL-BOLD-20"
			        :background (if (even? (+ (quot (dec (brd d)) s) (rem (dec (brd d)) s))) (color 128 0 255) (color 190 190 250))
			        :bounds [(+ 40 (* j (inc (quot 300 s)))) (+ 35 (* i (inc (quot 300 s)))) (quot 300 s) (quot 300 s)]
                ...
                (button :text "New 3x3" :foreground :white :font "ARIAL-BOLD-18"
			:background :blue
		...
		(button :text "New 4x4" :foreground :white :font "ARIAL-BOLD-18"
			:background :blue
		...
		(button :text "New 5x5" :foreground :white :font "ARIAL-BOLD-18"
			:background :blue
			...
			:listen [:action (fn [e] (do
                ...
                (label  :text (format "Moves: %d" (:moves @x)) :foreground :black :font "ARIAL-BOLD-18"
